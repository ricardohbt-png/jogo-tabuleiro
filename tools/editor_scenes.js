...
(function () {
  ...
  function render(){
    ...
    root.querySelectorAll(".scene-event").forEach((card,i)=>{ card.dataset.order=String(i+1); addVisualControls(card,sc,root); bindEvent(card,sc,root); });
  ...
  function addVisualControls(card,sc,root){
    ...
    dup.type='button'; dup.dataset.dup=''; dup.textContent='⧉ Duplicar slide';
    box.appendChild(dup);
