...
(function () {
  ...
  function renderTavern() {
    ...
    root.innerHTML = '<div class="cityed"><header><div><h1>🏘️ Cidades e Lojas</h1><p>Edite o fundo, a máscara e as conversas dos grupos clicáveis da taverna.</p></div><button id="cityed-save" class="cityed-save">Salvar alterações</button></header>' + cityTabs() + cityButtons + (slots.length ? '<section class="cityed-tavern"><h2>🍺 Taverna de ' + esc(c.nome) + '</h2><p>Envie o fundo e a máscara. Cada cartão define um NPC/grupo clicável sobre essa máscara.</p>' + sceneConfig + slots.map(slot => '<article class="cityed-npc"><img src="../' + esc(scene.mask || slot.image) + '" alt=""><div><label>Nome<input data-npc-name="' + esc(slot.id) + '" value="' + esc(slot.name) + '"></label><label>Conversa<textarea data-npc-dialog="' + esc(slot.id) + '">' + esc(slot.dialog) + '</textarea></label><div class="cityed-coords"><label>X %<input type="number" min="0" max="100" step="0.1" data-npc-pos="x" data-npc-id="' + esc(slot.id) + '" value="' + Number(slot.x) + '"></label><label>Y %<input type="number" min="0" max="100" step="0.1" data-npc-pos="y" data-npc-id="' + esc(slot.id) + '" value="' + Number(slot.y) + '"></label><label>Largura %<input type="number" min="1" max="100" step="0.1" data-npc-pos="w" data-npc-id="' + esc(slot.id) + '" value="' + Number(slot.w) + '"></label><label>Altura %<input type="number" min="1" max="100" step="0.1" data-npc-pos="h" data-npc-id="' + esc(slot.id) + '" value="' + Number(slot.h) + '"></label></div></div></article>').join('') + '</section>' : '<section class="cityed-tavern"><h2>🍺 Taverna de ' + esc(c.nome) + '</h2><p>Esta cidade ainda não possui uma cena de taverna configurada.</p></section>') + '<div id="cityed-status"></div></div>';
