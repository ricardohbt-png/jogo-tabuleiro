"""Recolor the orc miniature to match the supplied painted portrait."""
...
# The portrait uses a restrained, dark olive palette with nearly black hair,
# worn charcoal metal, deep leather/wood, and warm aged bone.
PALETTE = {
    "pele": ((0.050, 0.060, 0.022), 0.0, 0.78),
    "cabelo": ((0.004, 0.002, 0.001), 0.0, 0.94),
    "couro": ((0.024, 0.009, 0.004), 0.0, 0.90),
    "metal": ((0.045, 0.043, 0.039), 0.68, 0.52),
    "madeira": ((0.040, 0.014, 0.004), 0.0, 0.86),
    "osso": ((0.205, 0.125, 0.048), 0.0, 0.76),
    "terra": ((0.010, 0.006, 0.003), 0.0, 0.94),
    "capim": ((0.048, 0.034, 0.016), 0.0, 0.97),
    "disco": ((0.006, 0.005, 0.004), 0.0, 0.92),
    "Material": ((0.011, 0.008, 0.005), 0.0, 0.92),
}
...
def recolor(meshes):
    ...
    for obj in meshes:
        for mat in obj.data.materials:
            ...
            # Blender may suffix the generic pedestal material on import.
    ...
    for obj in meshes:
        obj["palette_reference"] = "assets/retratos/monstros/orc.png"
        obj["palette_adjustment"] = (
            "dark olive skin, black hair, worn charcoal metal, deep leather and wood, "
            "warm aged bone"
        )
...
def make_preview(meshes, output: Path):
    ...
    # Three-quarter view keeps the face, shoulder armor, fur, and shield readable.
    bpy.ops.object.camera_add(
        location=(height * 2.35, -height * 3.1, center.z + height * 0.92)
    )
    ...
    for location, energy, size in [
        ((height * 2.0, -height * 3.0, center.z + height * 2.7), 980, height * 1.8),
        ((-height * 2.0, -height * 1.0, center.z + height * 1.35), 380, height * 1.6),
    ...
    scene.world = bpy.data.worlds.new("Orc preview world")
