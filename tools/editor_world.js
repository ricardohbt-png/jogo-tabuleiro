...
(function () {
  ...
  function render() {
    ...
    $('#worlded-add',root).onclick=()=>{let n=1,id; do{id='destino_'+n++;}while(config.adventures.some(x=>x.id===id)); config.adventures.push({id:id,nome:'Novo destino',x:50,y:50,fome:0,sede:0,dungeons:[],espera_retorno:{modo:'fixa',rodadas:0,dados:''},oculto_ate_liberar:false,requisito:{tipo:'nenhum',valor:''}});selected=id;render();};
