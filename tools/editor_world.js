...
(function () {
  ...
  function render() {
    ...
    $('#worlded-add',root).onclick=()=>{let n=1,id; do{id='destino_'+n++;}while(config.adventures.some(x=>x.id===id)); config.adventures.push({id:id,nome:'Novo destino',x:50,y:50,fome:0,sede:0,dungeons:[],espera_retorno:{modo:'fixa',rodadas:0,dados:''},oculto_ate_liberar:false,revisitavel:false,outro_rota:'',requisito:{tipo:'nenhum',valor:''}});selected=id;render();};
  ...
  function form(root,a) {
    ...
    // Rota anterior exigida: dropdown dos OUTROS destinos (mostra o nome, salva o id
    // do ponto do mapa — é o id que _avaliar_requisito procura em WORLD_ADVENTURES).
    // Um id órfão (destino removido/renomeado, ou nome digitado à mão no formato
    // antigo de texto livre) vira uma opção marcada, para não sumir em silêncio.
    const reqRota=String(req.aventura_id||'').trim();
    const outrosDest=(config.adventures||[]).filter(x=>x.id!==a.id);
    const rotaOpts=['<option value="">— nenhuma —</option>'].concat(
      outrosDest.map(x=>'<option value="'+esc(x.id)+'"'+(reqRota===x.id?' selected':'')+'>'+esc(x.nome||x.id)+' ('+esc(x.id)+')</option>'));
