...
(function () {
  ...
  function normalize(m, isCustom, overwriteNative=false) {
    ...
    out.monster_abilities = out.monster_abilities.map(a => {
      ...
      if (b.id === "explosao_vapor") { b.damage_dice = n(b.damage_dice, 4); b.damage_faces = n(b.damage_faces, 6); b.range = 2; b.shape = "circle"; b.area_raio = 2; b.dc = n(b.dc, 16); b.cooldown_turns = 6; }
  ...
  function read() {
    ...
    out.monster_abilities = [...root.querySelectorAll(".me-ability:checked")].map(el => {
      ...
      if (el.value === "explosao_vapor") {
        ...
        config.range = 2; config.shape = "circle"; config.area_raio = 2; config.save = "reflexos"; config.cooldown_turns = 6;
      }
      ...
      if (el.value === "sacudida_brutal" || el.value === "sacudida_jurassica") {
        const shakeDice = card.querySelector(".me-tirano-shake-dice");
        if (shakeDice) {
          config.damage_dice = Math.max(1, Math.min(20, n(shakeDice.value, 4)));
          ...
          config.throw_distance = Math.max(1, Math.min(8, n(card.querySelector(".me-tirano-shake-distance").value, 4)));
          config.bite_damage = card.querySelector(".me-tirano-shake-bite")?.value?.trim() || "";
          config.extra_damage = card.querySelector(".me-tirano-shake-extra")?.value?.trim() || "2d6";
          config.wall_damage = card.querySelector(".me-tirano-shake-wall")?.value?.trim() || "2d6";
        }
      }
      if (el.value === "engolir") {
  ...
  function updateCalculated() {
    ...
    if (spellDc) spellDc.innerHTML = `1º: ${8 + mod(m.int_) + 1} · 2º: ${8 + mod(m.int_) + 2} · 3º: ${8 + mod(m.int_) + 3} · 4º: ${8 + mod(m.int_) + 4} · 5º: ${8 + mod(m.int_) + 5}`;
  ...
  function render() {
    ...
    const abilityCards = abilitySources.map(source => {
      ...
      const molochusSteamConfig = a.id === "explosao_vapor" ? `<span class="me-ability-limit">dados<input class="me-molochus-steam-dice" type="number" min="1" max="20" value="${esc(cfg.damage_dice || 4)}"></span><span class="me-ability-limit">faces<select class="me-molochus-steam-faces">${[4,6,8,10,12,20].map(v=>`<option value="${v}"${Number(cfg.damage_faces || 6)===v ? " selected" : ""}>d${v}</option>`).join("")}</select></span><span class="me-ability-limit">CD<input class="me-molochus-steam-dc" type="number" min="1" max="40" value="${esc(cfg.dc || 16)}"></span><small>círculo fixo de raio 2 · recarga fixa 6</small>` : "";
        ...
        const tiranoShakeConfig = (a.id === "sacudida_brutal" || a.id === "sacudida_jurassica")
          ? ((a.id === "sacudida_jurassica" || draft.type === "tiranossauro_rex" || cfg.bite_damage)
            ? `<span class="me-ability-limit">mordida<input class="me-tirano-shake-bite" type="text" maxlength="24" value="${esc(cfg.bite_damage || "4d12+7")}"></span><span class="me-ability-limit">extra<input class="me-tirano-shake-extra" type="text" maxlength="12" value="${esc(cfg.extra_damage || "2d6")}"></span><span class="me-ability-limit">parede<input class="me-tirano-shake-wall" type="text" maxlength="12" value="${esc(cfg.wall_damage || "2d6")}"></span><span class="me-ability-limit">arremessa<input class="me-tirano-shake-distance" type="number" min="1" max="8" value="${esc(cfg.throw_distance || 4)}"></span><small>dano da Mordida + extra; parede acrescenta o dano indicado</small>`
            : `<span class="me-ability-limit">dados<input class="me-tirano-shake-dice" type="number" min="1" max="20" value="${esc(cfg.damage_dice || 4)}"></span><span class="me-ability-limit">faces<select class="me-tirano-shake-faces">${[4,6,8,10,12,20].map(v=>`<option value="${v}"${Number(cfg.damage_faces || 6)===v ? " selected" : ""}>d${v}</option>`).join("")}</select></span><span class="me-ability-limit">arremessa<input class="me-tirano-shake-distance" type="number" min="1" max="8" value="${esc(cfg.throw_distance || 2)}"></span><small>recarga definida pela ficha</small>`)
          : "";
    ...
    if (casterBox) casterBox.insertAdjacentHTML("beforeend", `<div class="me-calculated me-spell-dc"><b>CD de resistência</b><span id="me-spell-dc-final">8 + INT + círculo</span><small>1º: ${8 + mod(draft.int_) + 1} · 2º: ${8 + mod(draft.int_) + 2} · 3º: ${8 + mod(draft.int_) + 3} · 4º: ${8 + mod(draft.int_) + 4} · 5º: ${8 + mod(draft.int_) + 5}</small></div>`);
