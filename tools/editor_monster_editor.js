...
(function () {
  ...
  function normalize(m, isCustom, overwriteNative=false) {
    ...
    out.monster_abilities = out.monster_abilities.map(a => {
      ...
      if (b.id === "sangue_em_ebulicao") { b.damage_dice = n(b.damage_dice, 1); b.damage_faces = n(b.damage_faces, 4); }
      ...
      if (b.id === "morte_explosiva") { b.damage_dice = n(b.damage_dice, 4); b.damage_faces = n(b.damage_faces, 6); b.radius = n(b.radius, 2); b.dc = n(b.dc, 16); b.duration = n(b.duration, 2); }
      return b;
    });
  ...
  function read() {
    ...
    out.monster_abilities = [...root.querySelectorAll(".me-ability:checked")].map(el => {
      ...
      if (el.value === "carapaca_espinhosa") {
        config.damage_dice = Math.max(1, Math.min(20, n(card.querySelector(".me-lacralion-shell-dice").value, 1)));
        config.damage_faces = Math.max(4, Math.min(20, n(card.querySelector(".me-lacralion-shell-faces").value, 4)));
      }
      ...
      if (el.value === "sangue_em_ebulicao") {
        config.damage_dice = Math.max(1, Math.min(20, n(card.querySelector(".me-molochus-blood-dice").value, 1)));
        config.damage_faces = Math.max(4, Math.min(20, n(card.querySelector(".me-molochus-blood-faces").value, 4)));
      }
  ...
  function render() {
    ...
    const abilityCards = abilitySources.map(source => {
      const cards = positiveAbilities.filter(source.filter).map(a => {
        ...
        const molochusBloodConfig = a.id === "sangue_em_ebulicao" ? `<span class="me-ability-limit">dados<input class="me-molochus-blood-dice" type="number" min="1" max="20" value="${esc(cfg.damage_dice || 1)}"></span><span class="me-ability-limit">faces<select class="me-molochus-blood-faces">${[4,6,8,10,12,20].map(v=>`<option value="${v}"${Number(cfg.damage_faces || 4)===v ? " selected" : ""}>d${v}</option>`).join("")}</select></span><small>apenas acerto corpo a corpo</small>` : "";
        ...
        const molochusDeathConfig = a.id === "morte_explosiva" ? `<span class="me-ability-limit">dados<input class="me-molochus-death-dice" type="number" min="1" max="20" value="${esc(cfg.damage_dice || 4)}"></span><span class="me-ability-limit">faces<select class="me-molochus-death-faces">${[4,6,8,10,12,20].map(v=>`<option value="${v}"${Number(cfg.damage_faces || 6)===v ? " selected" : ""}>d${v}</option>`).join("")}</select></span><span class="me-ability-limit">raio<input class="me-molochus-death-radius" type="number" min="1" max="10" value="${esc(cfg.radius || 2)}"></span><span class="me-ability-limit">CD<input class="me-molochus-death-dc" type="number" min="1" max="40" value="${esc(cfg.dc || 16)}"></span><span class="me-ability-limit">chamas<input class="me-molochus-death-duration" type="number" min="1" max="10" value="${esc(cfg.duration || 2)}"></span>` : "";
