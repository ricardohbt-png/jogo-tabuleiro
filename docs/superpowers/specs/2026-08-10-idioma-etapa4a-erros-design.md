...
4. **a) Erros (471 ocorrências)** ← este · b) Narração (544)
5. Interface do cliente (~1.000)

**Erros antes da narração** porque é o lote mais seguro para assentar o método: 390 das 471
...
**Duas formas de chamada**, e a segunda quase me escapou: além do dicionário
`{"type": "error", "msg": …}`, o handler tem um atalho local `await err(…)`. O script de
migração precisa reconhecer as duas.

| Forma | Texto fixo | Com parâmetro | Total |
|---|---:|---:|---:|
| `"msg": "…"` | 382 | 80 | 462 |
| `await err("…")` | 8 | 1 | 9 |
| **Total** | **390** | **81** | **471** |

Os 390 de texto fixo deduplicam para **306 textos distintos**.
...
| Migração dos 390 de texto fixo | Script determinístico, diff conferível |
| Migração dos 81 com parâmetro | À mão |
...
**Os 390 de texto fixo, por script.** Um script varre `server.py`, e para cada literal
`"msg": "…"` **ou** `await err("…")` gera o slug, reescreve a chamada para `"msg": T("erro.…")` e acumula a entrada
...
**Os 81 com parâmetro, à mão.** Cada um exige decidir o nome do parâmetro
...
1. **Nenhum literal de erro sobrou no `server.py`** — nem `"msg": "…"` nem `await err("…")`. É a prova de migração completa —
...
4. **Paridade de parâmetros entre `pt` e `en`**, em TODAS as chaves de TODOS os arquivos de
   ...
   protege as 81 desta etapa e as 399 da narração — é a falha mais provável numa tradução em
...
Entre 306 e 387 chaves curtas — 306 vêm do texto fixo já deduplicado, e as parametrizadas
somam no máximo 81, provavelmente menos porque também se repetem. O número exato sai do
